...
<style>
    ...
    .q-box { background-color: #f8f9fa; padding: 10px; border-radius: 5px; margin-bottom: 5px; }
</style>
...
if st.session_state["active_plan_flag"]:
    ...
    for i in range(1, 5):
        ...
        with st.container():
            st.markdown(f'<div class="q-box"><b>{i}Q</b> (Ave: {int(q_sec//60)}:{q_sec%60:02.1f})</div>', unsafe_allow_html=True)
    ...
    if calc_mode == 'distance_base':
        ...
        st.write(f"### 合計タイム: {int(p_total_secs//60)}:{p_total_secs%60:02.1f}")
