...
# セッション状態（状態保持用）に計算のベースとなる数値を保存する
if "current_base_ave" not in st.session_state:
    st.session_state["current_base_ave"] = 0.0
if "current_distance" not in st.session_state:
    st.session_state["current_distance"] = 0.0
if "current_target_seconds" not in st.session_state:
    st.session_state["current_target_seconds"] = 0.0
...
# 確定した計算結果をセッション状態に退避させる（★ここが修正の肝です）
st.session_state["current_base_ave"] = base_ave_seconds
st.session_state["current_distance"] = distance
st.session_state["current_target_seconds"] = target_total_seconds
...
if st.session_state["plan_generated"]:
    # 退避させておいた計算結果をレースプラン作成用に使用する
    saved_base_ave = st.session_state["current_base_ave"]
    saved_distance = st.session_state["current_distance"]
    saved_target_seconds = st.session_state["current_target_seconds"]

    if is_time_based:
        q_info = f"各Qの長さ（時間固定）: {int((saved_target_seconds/4)//60)}分 {int((saved_target_seconds/4)%60)}秒"
    else:
        q_info = f"各Qの長さ（距離固定）: {saved_distance/4:.0f}m"
        
    st.subheader(f"⏱️ 各Q의 調整 (500m Average)")
    st.caption(f"💡 {q_info}")
    ...
    for i in range(1, 5):
        ...
        # 計算された正しいAverage（saved_base_ave）に、ボタンの増減分を足す
        q_seconds = saved_base_ave + st.session_state[f"calc_q{i}_diff"]
        if q_seconds < 0:
            q_seconds = 0.0
    ...
    if not is_time_based:
        ...
        diff = plan_total_seconds - saved_target_seconds
        ...
        q_duration = saved_target_seconds / 4
        plan_distance = 0.0
        for q_sec in q_times:
            if q_sec > 0:
                plan_distance += (q_duration / q_sec) * 500
                
        diff_dist = plan_distance - saved_distance
        st.write(f"現在のプラン合計距離: **{plan_distance:.1f} m** （目標: {saved_distance:.1f} m）")
