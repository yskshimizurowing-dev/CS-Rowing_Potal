...
# スマホでも見やすい背景色付きスタイルを追加
st.markdown("""
<style>
    .stButton>button { width: 100%; height: 35px; font-size: 13px; font-weight: bold; }
    .q-container { background-color: #f0f2f6; padding: 10px; border-radius: 10px; margin-bottom: 10px; }
...
with st.expander("①～⑤ 設定入力と作成", expanded=not st.session_state["active_plan_flag"]):
    ...
    st.write("時間/Ave (分:秒)")
    c1, c2 = st.columns(2)
    m, s = c1.number_input("分", value=2), c2.number_input("秒", value=0)
    
    if st.button("プラン作成・更新", type="primary"):
        calc_secs = m * 60 + s
        calc_ave = calc_secs / (dist / 500) if mode_idx in [0, 2] else calc_secs
        calc_mode = 'distance_base' if mode_idx < 2 else 'time_base'
        if mode_idx == 1: calc_secs = calc_ave * (dist / 500)
...
if st.session_state["active_plan_flag"]:
    ...
    for i in range(1, 5):
        ...
        # 背景色付きコンテナを使用
        with st.container():
            st.markdown(f'<div class="q-container">', unsafe_allow_html=True)
