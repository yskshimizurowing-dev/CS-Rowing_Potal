...
if st.session_state["active_plan_flag"]:
    ...
    for i in range(1, 5):
        ...
        # 背景付きコンテナ（border=Trueで標準的な枠組みを表示）
        with st.container(border=True):
            if calc_mode == 'distance_base':
                this_v = q_sec * ((dist_total/4)/500)
                st.write(f"**{i}Q** Ave {int(q_sec//60)}:{q_sec%60:04.1f}   |   {int(this_v//60)}:{this_v%60:04.1f}")
                p_total_secs += this_v
            else:
                this_v = (secs_total/4/q_sec)*500 if q_sec > 0 else 0
                st.write(f"**{i}Q** Ave {int(q_sec//60)}:{q_sec%60:04.1f}   |   {this_v:.1f}m")
                p_total_dist += this_v
    ...
    if calc_mode == 'distance_base':
        ...
        st.write("目標通り" if abs(diff) < 0.5 else ("遅い" if diff > 0 else "速い"))
    else:
        diff = p_total_dist - dist_total
        st.write(f"合計距離: {p_total_dist:.1f}m")
        st.write("目標通り" if abs(diff) < 0.5 else ("多い" if diff > 0 else "不足"))
