...
# スマホでもボタンが押しやすいよう調整したCSS
st.markdown("""
<style>
    .stButton>button { width: 100%; height: 35px; font-size: 12px; font-weight: bold; }
...
with st.expander("①～⑤ 設定入力とプラン作成", expanded=not st.session_state["active_plan_flag"]):
    ...
    if mode_idx in [0, 1]:
        ...
        c1, c2 = st.columns(2)
        m = c1.number_input("分", value=8 if mode_idx==0 else 2)
        ...
        c1, c2 = st.columns(2)
        m = c1.number_input("合計分", value=20); s = c2.number_input("秒", value=0)
    ...
    if st.button("プラン作成", type="primary"):
        st.session_state.update({
            ...
            "fixed_distance_m": calc_dist, "fixed_total_seconds": calc_secs,
...
if st.session_state["active_plan_flag"]:
    ...
    for i in range(1, 5):
        ...
        if calc_mode == 'distance_base':
            ...
            res_str = f"タイム: {int(this_v//60)}:{this_v%60:04.1f}"
            p_total_secs += this_v
        else:
            this_v = (secs_total/4/q_sec)*500 if q_sec > 0 else 0
            res_str = f"距離: {this_v:.1f}m"
            p_total_dist += this_v
        
        c1, c2, c3 = st.columns([2, 1, 1])
        c1.write(res_str)
        if c2.button("➕", key=f"p{i}"): st.session_state[f"q{i}_off"] += 0.5; st.rerun()
        if c3.button("➖", key=f"m{i}"): st.session_state[f"q{i}_off"] -= 0.5; st.rerun()
    ...
    if calc_mode == 'distance_base':
        ...
        st.write(f"### 合計: {int(p_total_secs//60)}:{p_total_secs%60:04.1f}")
        st.write("🎉 OK!" if abs(diff)<0.1 else (f"⚠️ {diff:.1f}秒遅い" if diff>0 else f"💡 {abs(diff):.1f}秒速い"))
    else:
        diff = p_total_dist - dist_total
        st.write(f"### 合計: {p_total_dist:.1f}m")
        st.write("🎉 OK!" if abs(diff)<0.5 else (f"🚀 {diff:.1f}m多い" if diff>0 else f"⚠️ {abs(diff):.1f}m不足"))
